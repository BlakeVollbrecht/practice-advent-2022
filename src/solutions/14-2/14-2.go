...
func solve(input []string) {
	...
	absolute_drop_position := Point{x: ABSOLUTE_DROP_POSITION_X, y: ABSOLUTE_DROP_POSITION_Y}

	_, max_corner := getBounds(paths)
	height := max_corner.y + 3

	grid, offset_x := makeGrid(absolute_drop_position, height, paths)
	...
	sand_count := 0
	for sand_flowing {
		sand_flowing = dropSand(grid, drop_position)
		sand_count++
	}

	drawGrid(grid)

	// sanity := countSand(grid)

	// fmt.Printf("Sand count: %d, %d\n", sand_count, sanity)
...
func dropSand(grid [][]byte, drop_position Point) bool {
	...
	if sand.x == drop_position.x && sand.y == drop_position.y {
		return false
	}
...
func makeGrid(drop_position Point, height int, paths [][]Point) ([][]byte, int) {
	offset_x := drop_position.x - (height - 1)

	grid := make([][]byte, height)

	for i := 0; i < height; i++ {
		grid[i] = make([]byte, height*2-1) // angle allows pile to be twice as wide as tall (minus duplicate center column)
	}
	...
	for i := 0; i < len(grid[len(grid)-1]); i++ {
		grid[len(grid)-1][i] = '#' // add floor to entire bottom row
	}
