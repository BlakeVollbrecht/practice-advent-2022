...
pub fn solve() !void {
    ...
    const least_steps = try travel(map, distance_grid, start);
...
fn travel(map: ArrayList([]const u8), distance_grid: ArrayList(ArrayList(u32)), start: @Vector(2, u32)) !u32 {
    ...
    while (possible_moves.items.len > 0) {
        ...
        for (possible_moves.items) |move| {
            ...
            if (map.items[y][x] == 'a') {
                return distance_grid.items[y].items[x];
            }
...
fn getPossibleMoves(map: ArrayList([]const u8), distance_grid: ArrayList(ArrayList(u32)), current_coords: @Vector(2, u32)) !ArrayList(@Vector(2, u32)) {
    ...
    if (offNorthEdge) {
        ...
        const next_height_fine = current_height <= next_height + 1;
