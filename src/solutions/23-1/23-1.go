...
// Notes:
// - since movement is restricted to 1 space per round and number of rounds are restricted, working area only needs to be at most the input size plus a margin with width equal to the number of rounds
// - map the proposed moves keyed by position and when a collision occurs, add both elf id's to an array of halted elves (leave the first one in the map)
// - can have elves as map[id]position, map[int]int keyed off scanline position holding elf id for moves, []int for halted elf ids, and make a [][]bool grid of the area at start of round from the elf positions
...
func solve(input []string) {
	...
	// grid, _ := makeElfGrid(&elves, NUM_ROUNDS-1)
	// drawGrid(&grid)

	nwBound, seBound := getElfBounds(&elves)
	boundsLengthY := seBound.y - nwBound.y + 1
	boundsLengthX := seBound.x - nwBound.x + 1
	boundArea := boundsLengthY * boundsLengthX

	answer := boundArea - len(elves)
	fmt.Printf("Answer: %d\n", answer)
}

func moveElves(elves *map[int]Coordinates, roundCounter int) {
...
func makeElfGrid(elves *map[int]Coordinates, roundCounter int) ([][]bool, Coordinates) {
	nwBound, seBound := getElfBounds(elves)
	boundsLengthY := seBound.y - nwBound.y + 1 + 2*MOVE_DISTANCE // margin added on all sides; enough for one round only
	boundsLengthX := seBound.x - nwBound.x + 1 + 2*MOVE_DISTANCE

	grid := make([][]bool, boundsLengthY)
	for i := range grid {
		grid[i] = make([]bool, boundsLengthX)
	}
