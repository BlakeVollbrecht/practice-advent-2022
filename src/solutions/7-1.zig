...
pub fn solve() !void {
    ...
    var sum = sumSmallDirectories(100000, &nodes, "/");
...
fn sumSmallDirectories(max_size: u32, nodes: *StringHashMap(Node), node_id: []const u8) u32 {
    var sum: u32 = 0;
    var node: Node = nodes.get(node_id) orelse unreachable;

    for (node.children.items) |child_id| {
        sum += sumSmallDirectories(max_size, nodes, child_id);
    }
