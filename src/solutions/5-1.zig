...
pub fn solve() !void {
    ...
    while (try in_stream.readUntilDelimiterOrEof(&buf, '\n')) |line| {
        ...
        if (!onMoves) {
            ...
            try moveCrates(crates, count, source, dest);
        }
    }
...
fn moveCrates(crates: ArrayList(ArrayList(u8)), count: u32, source: u32, dest: u32) !void {
    ...
    while (j < count) {
        j += 1;
        try dest_stack.append(source_stack.pop());
    }
}
